...
/**
 * This utility class provides functions to look up configurations from json objects, to handle broker configurations
 * and provides even further utility functions.
 */
@Slf4j
@UtilityClass
public class ConfigUtility {
    ...
    /**
     * The reference for the default broker configuration name in the configurations.
     */
    ...
    /**
     * Tries to look up the value specified under the key in the clientConfig object. If the clientConfig object is
     * no json object or no entry exists under the key than the default value will be returned.
     * @param clientConfig the object to look up the referenced value
     * @param key the key to look up the value in the json object
     * @param defaultConfig the value returned if the referenced object does not exist
     * @return returns the referenced value if existing or else the defaultConfig object
     */
    ...
    /**
     * Looks up the referenced value in the clientConfig object. If no value exists or the clientConfig object
     * is no json object than null will be returned.
     * @param clientConfig the object to look up the referenced value
     ...
     * @param pipMqttClientConfigVal {@link Val} of the configuration in the attribute finder
     * @return Returns a json object containing the mqtt broker config. If no valid configuration was provided
     * in the configurations than a {@link NoSuchElementException} will be thrown.
     */
    ...
    private static ObjectNode getBrokerConfigFromPdpConfig(JsonNode pipMqttClientConfig, ObjectNode mqttPipBrokerConfig,
        ...
        if (brokerConfig != null) { // broker config is specified in pdp.json
            ...
            } else if (pipMqttClientConfigVal.isTextual()) { // if attribute finder contains reference to configDescription in pdp.json
