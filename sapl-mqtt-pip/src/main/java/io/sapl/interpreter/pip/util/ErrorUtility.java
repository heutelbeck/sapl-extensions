...
public class ErrorUtility {
    ...
    /**
     * The reference for the status in the configuration whether to emit a value at retry or not.
     */
    ...
    private static final boolean DEFAULT_EMIT_AT_RETRY = true; // if true then an undefined value will be emitted at retry
    ...
    public static RetryBackoffSpec getRetrySpec(JsonNode pipMqttClientConfig) {
        return Retry
                ...
                .doBeforeRetry(retrySignal-> {
                    ...
                    log.info("Trying to reestablish connection and subscription. Retry number " +
    ...
    /**
     * Evaluates the {@link Throwable} whether the broker config hash has to be removed from the client cache or not.
     * @param throwable the {@link Throwable} to evaluate
     * @return returns true if the broker config hash has to be removed from the client cache
     */
