...
public class MqttTestUtil {
    ...
    private static EmbeddedHiveMQ buildBroker(Path configDir, Path dataDir, Path extensionsDir,
        ...
        EmbeddedHiveMQ broker = EmbeddedHiveMQ.builder().withConfigurationFolder(configDir).withDataFolder(dataDir)
                .withExtensionsFolder(extensionsDir).withEmbeddedExtension(embeddedExtensionBuild).build();
    ...
    private static EmbeddedExtension buildExtension(ExtensionMain hiveMqPepExtensionMain) {
    ...
    private static Mqtt5BlockingClient buildMqttClient(String mqttClientId, int mqttServerPort) {
        return Mqtt5Client.builder().identifier(mqttClientId).serverHost(BROKER_HOST).serverPort(mqttServerPort)
                .buildBlocking();
    }
    ...
    public static Mqtt5Subscribe buildMqttSubscribeMessage(String topic, int qos) {
        return Mqtt5Subscribe.builder().topicFilter(topic).qos(Objects.requireNonNull(MqttQos.fromCode(qos))).build();
    }
    ...
    public static Mqtt5Publish buildMqttPublishMessage(String topic, int qos, boolean retain, String contentType) {
        return Mqtt5Publish.builder().topic(topic).qos(Objects.requireNonNull(MqttQos.fromCode(qos))).retain(retain)
                .contentType(contentType).payload(PUBLISH_MESSAGE_PAYLOAD.getBytes(StandardCharsets.UTF_8)).build();
    }
}
