 ...
 * singlesubscription using VaadinSingleButtonPepBuilder and the right three are
 * for multisubscriptions with MultiBuilder
 */
@PermitAll
...
public class SingleAndMultisubscriptionPage extends VerticalLayout {
    ...
    private VerticalLayout createSingleSubscriptionLayout(PepBuilderService pepBuilderService) {
        ...
        // add info
        singleSubscriptionLayout
                .add(Utilities.getInfoText("access to buttons is granted/revoked in a 5 seconds interval "
    ...
    private Button createButtonSingleSubscriptionEnforce(PepBuilderService pepBuilderService) {
        var resource = JsonNodeFactory.instance.objectNode().put(OBJECT, "button").put("page",
                SINGLE_AND_MULTISUBSCRIPTION);

        return pepBuilderService.with(new Button("enforced by singlesubscription")).action(VIEW_COMPONENT)
    ...
    private VerticalLayout crateMultisubscriptionLayout(PepBuilderService pepBuilderService) {
        ...
        var textfieldResource = JsonNodeFactory.instance.objectNode().put(OBJECT, "textfield").put("page",
                SINGLE_AND_MULTISUBSCRIPTION);
