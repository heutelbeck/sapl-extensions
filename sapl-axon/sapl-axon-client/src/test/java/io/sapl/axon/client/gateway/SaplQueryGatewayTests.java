...
public class SaplQueryGatewayTests {
    ...
    void setUp() {
        ...
        AbstractSaplQueryInterceptor abstractSaplQueryInterceptor = mock(AbstractSaplQueryInterceptor.class);
        when(abstractSaplQueryInterceptor.handle(isA(QueryMessage.class))).thenAnswer(i -> i.getArguments()[0]);
    ...
    void buildSaplQueryGatewayWithNullDispatchInterceptor() {
        SaplQueryGateway.builder().queryBus(mockBus).dispatchInterceptors(new CopyOnWriteArrayList<>())
                .abstractSaplQueryInterceptor(mock(AbstractSaplQueryInterceptor.class)).build();
    }
